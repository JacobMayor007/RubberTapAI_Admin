...
const BASE_URL = import.meta.env.VITE_API_URL || "https://rubbertapai-backend.vercel.app";
// VITE_ADMIN_API_KEY is: 4ca0ddb5bb2f348ad2f5e2c7ce531d8c
...
export default function UserManagement() {
    ...
    /**
     * Fetches the main list of reported users and all associated reports.
     * Uses: POST /api/v1/admin/reports
     */
    const fetchReportedUsers = useCallback(async () => {
        setLoading(true);
        setError(null);
        try {
            if (!API_KEY) throw new Error("API_KEY is not configured in environment variables.");
            ...
            const groupedUsers = data.reduce((acc, report) => {
                ...
                if (!acc[id]) {
                    acc[id] = {
                        ...
                        status: report.status && report.status.toLowerCase() === "disable" ? "Disabled" : "Enabled",
                        reports: [], 
                    };
                }
            ...
            setError(`CRITICAL API ERROR: ${err.message}. Please check your Admin API Key and Appwrite User status.`);
            setUserReportsList([]);
        } finally {
            setLoading(false);
        }
     ...
     */
    const fetchReportersForUser = useCallback((reportedId) => {
    ...
    const handleToggleStatus = async (id, currentStatus, e) => {
        ...
        try {
            ...
            if (!API_KEY) {
                throw new Error("Missing VITE_ADMIN_API_KEY. Check your .env configuration.");
            }
            ...
            if (!response.ok) {
                const errorMessage = result.message || `Server returned HTTP ${response.status}.`;
                console.error(`Status Toggle Failed (HTTP ${response.status}):`, errorMessage, result);
            ...
            if (result.success) {
                ...
                setNotif({
                    message: result.message || `${targetName} is now ${newStatusDisplay}.`,
                    type: "success",
                });
            } else {
                 // FAILURE: Backend returned 200 OK but success: false
                 const errorMessage = result.message || `Failed to set status to ${newStatusDisplay}. Reason unknown.`;
                 console.error(`Status Toggle Failed (Success: False):`, errorMessage, result);
                 throw new Error(errorMessage);
            }
        } catch (err) {
        ...
        <table className="w-full text-left table-fixed">
            <thead>
                <tr>
                    ...
                    <th className="py-2 text-[#4B2E1E] font-semibold w-[25%]">Account Name</th> 
                    <th className="py-2 text-[#4B2E1E] font-semibold w-[15%]">Date</th> 
                    <th className="py-2 text-[#4B2E1E] font-semibold w-[45%]">Report Reason</th> 
                </tr>
            </thead>
            <tbody>
                {reports.map((report) => (
                    ...
                    <div className="space-y-6">
                        ...
                        {filteredUsers.map((user) => (
                            <div key={user.id}>
                                {/* Main User Row */}
                                <div
                                    className={`flex items-center justify-between rounded-xl bg-white px-6 py-4 shadow cursor-pointer transition ${expandedUserId === user.id ? 'ring-2 ring-offset-2 ring-[#C2A78C]' : ''}`}
                                        ...
                                        {expandedReports.length > 0 ? (
                                            <>
                                                ...
                                                <div className="mt-4 text-md text-[#4B2E1E] font-semibold">
                                                    List of accounts who reported this user ({expandedReports.length} total reports).
                                                </div>
                                            </>
                                        ) : (
